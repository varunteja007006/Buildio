...
function FolderRow({
  ...
  const folderDocs = folderData?.pages.flatMap((p) => p.documents) ?? node.documents
        ...
        <CollapsibleContent>
          <ul>
            ...
            {isOpen && (
              <>
                {folderLoading ? (
                  <li className="flex items-center gap-2 px-2 py-1.5 text-xs text-muted-foreground" style={{ paddingLeft: `${(depth + 1) * 1.25 + 0.5}rem` }}>
...
function PaginatedFileList({
  ...
  useEffect(() => {
    if (docs.length < visible) setVisible(Math.max(FOLDER_PAGE_SIZE, docs.length))
  }, [docs.length, visible])

  const visibleDocs = docs.slice(0, visible)
  const canShowMoreLocal = visible < docs.length
  const canFetchMoreGlobal = !canShowMoreLocal && !!hasNextPage && docs.length > 0
  ...
  }, [canShowMoreLocal, canFetchMoreGlobal, isFetchingNextPage, docs.length, fetchNextPage])
        ...
        >
          {isFetchingNextPage ? (
            ...
            <Button
              ...
              onClick={() => setVisible((v) => Math.min(v + FOLDER_PAGE_SIZE, docs.length))}
            >
              Show more
            </Button>
          ) : (
