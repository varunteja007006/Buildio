  ...
  return useMutation(
    trpc.incomeSource.updateSource.mutationOptions({
      onSuccess: (data, variables) => {
        ...
        queryClient.invalidateQueries({
          queryKey: incomeSourceKeys.detail(variables.sourceId),
        });
        queryClient.invalidateQueries({ queryKey: incomeSourceKeys.lists() });
        options?.onSuccess?.();
      },
      onError: (error: any) => {
...
export function useDeleteIncomeSource(options?: {
  onSuccess?: () => void;
  onError?: (error: any) => void;
}) {
  const trpc = useTRPC();
  ...
  return useMutation(
    trpc.incomeSource.deleteSource.mutationOptions({
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({ queryKey: incomeSourceKeys.all });
        // Also invalidate incomes since they reference sources
