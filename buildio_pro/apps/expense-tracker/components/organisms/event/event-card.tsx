...
const ProgressSection = ({ event }: EventCardProps) => {
  const progress =
  ...
  return (
    <div className="space-y-2">
      ...
      <Progress
        ...
        className={cn(
          "h-2",
          progress > 100 ? "bg-destructive/20 [&>div]:bg-destructive" : "",
        )}
      />
...
const DatesSection = ({ event }: EventCardProps) => {
  return (
...
const AdditionalInfoSection = ({ event }: EventCardProps) => {
    ...
    <Card className="h-full transition-all hover:shadow-md pb-0">
      ...
      <CardContent className="flex-1 space-y-4">
        <div className="space-y-4">
          {/* Progress Section */}
          <ProgressSection event={event} />

          {/* Dates & Status */}
          <DatesSection event={event} />
        </div>

        <AdditionalInfoSection event={event} />
      </CardContent>
